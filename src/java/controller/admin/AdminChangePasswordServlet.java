...
public class AdminChangePasswordServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null && user.isGoogleUser()) {
            response.sendRedirect(redirect + "?pwError=" + URLEncoder.encode(
                    "Google account cannot change password. Use Google to sign in.",
        ...
        if (!ValidationUtil.isValidPassword(newPassword)) {
            response.sendRedirect(redirect + "?pwError=" + URLEncoder.encode(
                    "New password must be 6-128 characters with 1 uppercase letter and 1 number.",
                    StandardCharsets.UTF_8
            ));
            return;
        }
