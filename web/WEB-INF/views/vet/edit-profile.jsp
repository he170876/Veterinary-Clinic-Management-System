...
<head>
    ...
    <title>Vet Edit Profile - Anipats</title>
                ...
                <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="size-9 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm">
                            <%= displayName.length() > 0 ? displayName.substring(0, 1).toUpperCase() : "V" %>
                        </div>
                    </div>
                </div>
                <form method="post" action="<%= ctx %>/vet/edit-profile" enctype="multipart/form-data" class="p-6 space-y-6">
                    <div class="flex flex-col md:flex-row md:items-center gap-6 mb-4">
                        <div class="relative group">
                            <div id="profilePhotoPreview" class="w-24 h-24 rounded-full ring-4 ring-background-light dark:ring-slate-900 overflow-hidden flex items-center justify-center bg-primary/10 text-primary font-bold text-3xl shrink-0">
                                ...
                                <% } else { %>
                                <span id="profilePhotoInitial"><%= displayName.length() > 0 ? displayName.substring(0, 1).toUpperCase() : "V" %></span>
                                <% } %>
                            </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <a href="<%= ctx %>/vet/profile" class="px-5 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800">
                            Cancel
                        </a>
