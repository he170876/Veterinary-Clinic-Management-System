...
<head>
    ...
    <title>Admin Edit Profile - Anipats</title>
        ...
        <div class="p-6 flex flex-col h-full">
            <div class="space-y-8">
                ...
                <nav class="flex flex-col gap-2">
                    <a class="flex items-center gap-3 px-4 py-3 rounded-full transition-all bg-primary/10 text-primary border border-primary/20 hover:brightness-95"
                       href="${pageContext.request.contextPath}/owner/dashboard">
                        <span class="material-symbols-outlined">dashboard</span>
                        <span class="text-sm font-bold">Dashboard</span>
                    </a>

                    <a class="flex items-center gap-3 px-3 py-2.5 text-[#a17145] hover:bg-[#f4ede6] dark:hover:bg-gray-800 rounded-xl transition-all"
                       href="${pageContext.request.contextPath}/owner/user-management">
                        <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1">group</span>
                        <span class="text-sm font-semibold">User Management</span>
                    </a>

                    <a class="flex items-center gap-3 px-3 py-2.5 text-[#a17145] hover:bg-[#f4ede6] dark:hover:bg-gray-800 rounded-xl transition-all"
                       href="${pageContext.request.contextPath}/owner/services">
                        <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1">medical_services</span>
                        <span class="text-sm font-semibold">Services</span>
                    </a>

                    <a class="flex items-center gap-3 px-3 py-2.5 text-[#a17145] hover:bg-[#f4ede6] dark:hover:bg-gray-800 rounded-xl transition-all"
                       href="${pageContext.request.contextPath}/owner/images">
    ...
    <main class="flex-1 flex flex-col overflow-y-auto">
        <header class="sticky top-0 z-10 flex items-center justify-between bg-white dark:bg-[#1a0d0c] border-b border-[#e6dcdb] dark:border-[#3d2a29] px-8 py-4">
            ...
            <div class="flex items-center gap-4">
                <div class="relative">
                    ...
                    <div id="admin-edit-profile-menu"
        ...
        <div class="p-8 max-w-5xl mx-auto w-full">
            <div class="flex items-center gap-2 mb-6">
                <a class="text-[#896461] text-sm font-medium hover:text-primary transition-colors" href="<%= ctx %>/admin/profile">Dashboard</a>
            ...
            <div class="bg-white dark:bg-[#1a0d0c] rounded-2xl border border-[#e6dcdb] dark:border-[#3d2a29] overflow-hidden shadow-sm">
                <div class="p-8">
                    <form method="post" action="<%= ctx %>/admin/edit-profile" enctype="multipart/form-data" class="space-y-6">
                        <div class="flex flex-col md:flex-row md:items-center md:gap-6 mb-10 pb-10 border-b border-[#f4f0f0] dark:border-[#2d1a19]">
                            <div class="relative group">
                                <div id="profilePhotoPreview"
                                     class="w-32 h-32 rounded-full ring-4 ring-background-light dark:ring-[#2d1a19] overflow-hidden flex items-center justify-center bg-primary/10 text-primary font-bold text-4xl shrink-0">
                                    <% if (hasProfilePic) { %>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input name="fullName"
                                           class="w-full h-12 pl-10 pr-4 bg-white dark:bg-[#1a0d0c] border border-[#e6dcdb] dark:border-[#3d2a29] rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all text-[#181111] dark:text-white"
                                           type="text"
                                           value="<%= esc(user.getFullName() != null ? user.getFullName() : "") %>"
                                           minlength="1" maxlength="30"
                                           pattern="[a-zA-Z\u00C0-\u024F\u1E00-\u1EFF\s]{1,30}"
                                           title="1-30 characters, letters and spaces only (any language)."
                                           required/>
                                </div>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input name="phone"
                                           ...
                                           pattern="0[0-9]{9}"
                                           title="10 digits starting with 0 (e.g. 0123456789)."
                            ...
                            <div class="flex flex-col gap-2 md:col-span-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input class="w-full h-12 pl-10 pr-4 bg-[#f8f6f6] dark:bg-[#2d1a19] border border-[#e6dcdb] dark:border-[#3d2a29] rounded-xl text-[#896461] cursor-not-allowed"
                                           type="email"
                                           value="<%= esc(user.getEmail() != null ? user.getEmail() : "") %>"
                                           readonly/>
                                </div>
                                <p class="text-[#896461] text-xs mt-1">Email cannot be changed. Contact support if needed.</p>
                            </div>

                            <div class="flex flex-col gap-2 md:col-span-2">
                                <label class="text-[#181111] dark:text-white text-sm font-bold">Address</label>
                                <textarea name="address"
                                          class="w-full p-4 bg-white dark:bg-[#1a0d0c] border border-[#e6dcdb] dark:border-[#3d2a29] rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all text-[#181111] dark:text-white resize-none"
                                          placeholder="Street, city, postal code..."
                                          rows="3"
                                          maxlength="500"><%= esc(user.getAddress() != null ? user.getAddress() : "") %></textarea>
                                <p class="text-[#896461] text-xs mt-1">Optional. Max 500 characters.</p>
                            </div>
                        </div>

                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-[#f4f0f0] dark:border-[#2d1a19]">
                            <a href="<%= ctx %>/admin/profile"
                               class="px-6 h-11 border border-[#e6dcdb] dark:border-[#3d2a29] text-[#181111] dark:text-white font-bold rounded-xl hover:bg-white dark:hover:bg-[#2d1a19] transition-all inline-flex items-center justify-center">
                                Cancel
                            </a>
                            <button type="submit"
                                    class="px-8 h-11 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all">
                                Save Changes
                            </button>
                        </div>
                    </form>
...
<script>
    (function () {
        var toggle = document.getElementById('admin-edit-profile-toggle');
        var menu = document.getElementById('admin-edit-profile-menu');
