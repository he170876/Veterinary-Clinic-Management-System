...
<%
    ...
    String roleTitle = (user.getRole() != null && user.getRole().getRoleName() != null) ? user.getRole().getRoleName() : "Receptionist";
...
<head>
    ...
    <title>Receptionist Profile - Anipats</title>
    ...
    <script id="tailwind-config">
        tailwind.config = {
            ...
            theme: {
                extend: {
                    colors: { primary: "#ff7b00", "background-light": "#f8f7f5", "background-dark": "#23190f" },
...
<aside class="w-64 border-r border-slate-200 bg-white flex flex-col h-screen sticky top-0">
    ...
    <nav class="flex-1 px-4 mt-4 space-y-1">
        <a class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-50 transition-colors"
           href="${pageContext.request.contextPath}/Receptionist/Dashboard">
            <span class="material-symbols-outlined">dashboard</span>
            <span class="font-medium">Dashboard</span>
        </a>
        <a class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-50 transition-colors"
           href="${pageContext.request.contextPath}/Receptionist/ViewListAppointment">
            <span class="material-symbols-outlined">calendar_today</span>
            <span class="font-medium">Schedule</span>
        </a>
        <a class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-50 transition-colors"
           href="${pageContext.request.contextPath}/Receptionist/ManageAppointmentRequests">
...
<main class="flex-1 flex flex-col min-h-screen">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="relative">
                <button type="button"
                    ...
                    <% } else { %>
                    <span class="material-symbols-outlined text-primary bg-primary/10 w-full h-full flex items-center justify-center">person</span>
                    <% } %>
                </button>
    ...
    <div class="max-w-4xl mx-auto p-8">
        <% if ("1".equals(pwMsg)) { %>
        <div class="p-4 rounded-xl bg-green-50 border border-green-200 text-green-800 text-sm font-medium">
            Password updated successfully.
        </div>
        <% } %>
        <% if (pwErr != null && !pwErr.isEmpty()) { %>
        <div class="p-4 rounded-xl bg-red-50 border border-red-200 text-red-800 text-sm font-medium mt-4">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-start md:items-center gap-6">
            <div class="flex items-center gap-4">
                <% if (hasProfilePic) { %>
                <img src="<%= ctx %><%= profilePicUrl %>" alt="<%= displayName %>"
                     class="rounded-full size-20 border-4 border-white dark:border-slate-800 object-cover"/>
                <% } else { %>
            ...
            <div class="md:ml-auto flex gap-2">
                <a href="<%= ctx %>/Receptionist/edit-profile"
                   class="px-4 py-2 rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                    Edit Profile
                </a>
                <% if (!isGoogleUser) { %>
                <button type="button" id="btnChangePassword"
                        class="px-4 py-2 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors">
                    Change Password
                </button>
                <% } %>
            </div>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <dl class="space-y-3 text-sm">
                    <div><dt class="text-slate-400">Email</dt><dd class="text-slate-900 dark:text-slate-100 font-medium"><%= user.getEmail() %></dd></div>
                    <div><dt class="text-slate-400">Phone</dt><dd class="text-slate-900 dark:text-slate-100 font-medium"><%= (user.getPhone() != null && !user.getPhone().isEmpty()) ? user.getPhone() : "Not set" %></dd></div>
                    <div><dt class="text-slate-400">Address</dt><dd class="text-slate-900 dark:text-slate-100 font-medium"><%= (user.getAddress() != null && !user.getAddress().isEmpty()) ? user.getAddress() : "Not set" %></dd></div>
                </dl>
            </div>
            <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                <h2 class="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">
                    Update contact information and avatar from this page. Changing password uses the same validation rules as customers.
                </p>
            </div>
        </div>
    </div>
...
<div id="modalChangePassword" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
    <div class="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="px-6 pt-6 pb-4 flex items-center justify-between">
            ...
            <button type="button" id="btnCloseModal" class="text-slate-400 hover:text-slate-600">
                <span class="material-symbols-outlined">close</span>
            </button>
        </div>
        <form method="post" action="<%= ctx %>/Receptionist/change-password" class="px-6 pb-6 space-y-4">
            <div class="space-y-1">
            ...
            <div class="pt-2 flex justify-end gap-2">
                <button type="button" id="btnCancelModal"
                        class="px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
                    Cancel
                </button>
                <button type="submit"
